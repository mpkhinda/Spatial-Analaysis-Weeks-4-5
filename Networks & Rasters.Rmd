...
ggplot(transit_access) +
  ...
  geom_sf(data = transit_stop_locs, color = "black", size = 0.85) +
  geom_sf(data = transit_stop_locs, color = "white", size = 0.5) +
  coord_sf(crs = MA_state_plane) +
  theme_void()
...
ggplot(transit_access2) +
  geom_sf(aes(fill = accessibility), color = NA) +
  scale_fill_viridis_c(name = "Accessiblity score", option = "F") +
  geom_sf(data = Chelsea_streets, color = "white", alpha = 0.6, size = 0.3) +
